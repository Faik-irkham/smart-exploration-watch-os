#!/usr/bin/env python3
"""Catatan 6 Juli 2026 — GABUNGAN + hasil uji baru 5 Juli, gaya ilmiah namun
tetap terbaca orang awam. Salinan struktur catatan 3 Juli, dilengkapi: Sesi 5
(5 Juli, tersambung penuh), statistik agregat 7 batch dari log HR-METRIC,
validasi empiris rumus ukuran/frame, dan konfirmasi false-sent = 0 saat
tersambung.
...
OUT = ROOT / "docs" / "26-07-06-Catatan-Faik-BLE-SmartWatch-sederhana.docx"
FIG = ROOT / "figures"
...
body_p("Data diambil langsung dari kedua perangkat melalui koneksi adb (jalur penghubung antara komputer "
       ...
       "gagal terhubung (Sesi 2). Pada 5 Juli 2026 dilakukan satu sesi pengujian tambahan (Sesi 5, "
       "interval 3 menit, 23:07–23:40 WIB): berkas 2026-07-05_watch_hr_20260705_234006.csv (1.976 "
       "baris) dan M2101K6G_2026-07-05_phone_hr_20260705_234520.csv (1.974 baris), disertai log "
       "metrik HR-METRIC berisi tujuh baris tx_batch (watch_log_2026-07-05_23-37-57.txt).")
...
body_p("Perhitungan tersebut terkonfirmasi secara empiris pada pengujian 5 Juli 2026. Seluruh tujuh "
       "batch pada sesi itu berukuran seragam — 180 record, 8.461 byte (47,0 byte per record), dan 19 "
       "frame, persis hasil pembulatan ke atas dari 8.461 ÷ 508 = 16,7 (17 frame DATA ditambah START "
       "dan END) pada MTU 512 yang kembali disetujui penuh. Dengan tujuh batch, kinerja transfer kini "
       "dapat dilaporkan sebagai statistik: durasi rata-rata 121,5 ± 25,5 milidetik (rentang "
       "92,8–166,4) dan throughput rata-rata 70,3 ± 13,2 KB per detik — bukan lagi ilustrasi satu "
       "batch seperti pada evaluasi Juni.")
...
table([
    ...
    ("Total", "—", "420 mnt", "25.191", "22.429", "2.762", "89,04%"),
])
caption("Tabel 3. Hasil pengiriman per sesi (periode pengukuran).")
body_p("Secara keseluruhan lima sesi, smartwatch merekam 25.191 data dan 22.429 di antaranya sampai ke "
       "ponsel, sehingga tingkat keberhasilan pengiriman (delivery) mencapai 89,04% tanpa data ganda "
       "(duplikat). Sesi 5 — yang ponselnya tersambung penuh sepanjang sesi — mencapai 99,90%; dua "
       "data yang belum sampai hanyalah dua detik terakhir yang terekam setelah jadwal kirim "
       "terakhir (pending wajar, synced = 0). "
       ...
       "seluruh datanya.")
body_p("Perbandingan jumlah data yang diterima dan yang hilang pada empat sesi Juni ditampilkan pada "
       "Gambar 3 (Sesi 5 belum termasuk dalam figur; nilainya tercantum pada Tabel 3).")
...
table([
    ...
    ("3 (tinggi)", "23.063", "91,6%"),
    ("0 (sedang)", "15", "0,1%"),
    ("-1 (tanpa kontak)", "2.113", "8,4%"),
    ("Total", "25.191", "100%"),
])
caption("Tabel 4. Distribusi nilai akurasi sensor (periode pengukuran).")
body_p("Sebagian besar data (91,6%) memiliki kontak baik (accuracy 3); sisanya 8,4% tanpa kontak (-1) "
       "dan 0,1% kontak sedang (0). Pada 23.063 data dengan kontak baik, detak jantung terendah 60 bpm, "
       "tertinggi 123 bpm, dan rata-rata 83,5 bpm, dengan standard deviation 9,6 bpm (ukuran seberapa "
       "jauh nilai menyebar dari rata-rata; nilai sebesar ini masih wajar untuk aktivitas santai). "
       "Sesi 5 mencatat kualitas terbaik: seluruh pembacaannya (100%) berkontak baik.")
...
body_p("Sesi 5 (5 Juli) memberi konfirmasi penting atas temuan ini dari arah sebaliknya: pada sesi "
       "yang koneksinya tersambung penuh, tidak ditemukan satu pun data yang keliru ditandai terkirim "
       "(false-sent = 0) — log metrik menunjukkan seluruh tujuh batch menerima balasan ACK lengkap "
       "(180 record per batch), dan dua-satunya data yang belum sampai berstatus synced = 0 (pending "
       "wajar). Ini menegaskan bahwa penandaan prematur khusus terjadi pada kondisi koneksi terputus, "
       "bukan cacat yang selalu muncul.")

# ---------------- klarifikasi angka ----------------
head("Hubungan Angka 2.755, 2.760, dan 2.879")
body_p("Dalam analisis evaluasi Juni (empat sesi pertama) muncul tiga angka yang saling berkaitan. Agar tidak membingungkan, hubungan "
...
body_p("Timestamp tidak berasal dari sensor, melainkan dibuat aplikasi pada saat data disimpan (sekitar "
       ...
       "smartwatch–ponsel. Hasil uji lima sesi menunjukkan dari 25.191 data pengukuran, 22.429 diterima "
       "ponsel (89,04%) tanpa duplikat, dan isi data yang diterima 100% sama dengan catatan smartwatch. "
